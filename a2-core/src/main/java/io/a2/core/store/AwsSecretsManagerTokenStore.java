...
/**
 * AWS Secrets Manager backed TokenStore.
 *
 * Each token is stored as a secret named {@code a2/token/{tokenId}}.
 * Uses a thin {@link AwsSecretsClient} SPI so the AWS SDK is optional.
 *
 * Best for low-volume / compliance-sensitive tokens. High QPS → prefer Redis.
 */
public class AwsSecretsManagerTokenStore implements TokenStore {

    public interface AwsSecretsClient {
        void putSecret(String name, String secretString);
        Optional<String> getSecret(String name);
        void deleteSecret(String name);
        List<String> listSecrets(String namePrefix);
    }

    private final AwsSecretsClient client;
    ...
    public AwsSecretsManagerTokenStore(AwsSecretsClient client) {
        this(client, "a2/token/");
    }

    public AwsSecretsManagerTokenStore(AwsSecretsClient client, String prefix) {
        this.client = client;
    ...
    public void save(TokenRecord record) {
        client.putSecret(prefix + record.tokenId(), serialize(record));
    ...
    public Optional<TokenRecord> findById(String tokenId) {
        return client.getSecret(prefix + tokenId)
                .flatMap(AwsSecretsManagerTokenStore::deserialize)
                .filter(r -> !r.revoked())
